...
if uploaded_image is not None:
    ...
    # running DeepFace analysis for gender detection & displaying results in second column
    with col2:
        st.write("Gender Analysis Result:")
        try:
            result = DeepFace.analyze(img_np, actions=['gender'])

            # displaying each gender with its probability
            for gender, score in result[0]['gender'].items():
